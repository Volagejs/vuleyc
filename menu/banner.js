    ...
    run: async (interaction) => {
       ...
       .then((res) =>{
         ...
         if(banner){
           ...
           .setAuthor({
             name:`${target.user.username} Kişisine Ait Afiş`, iconURL:target.user.avatarURL({ dynamic: true, size: 1024 })
           })
           .setImage(url);
             interaction.reply({embeds:[embed],components:[new MessageActionRow().addComponents(but)]});
         }
         else{
           const embed2 = new MessageEmbed().setDescription('Kullanıcıya ait afiş bulunmuyor.').setColor(accent_color);
